...
app.get("/discounts", (req, res) => {
  ...
  var findDiscount = function() {
    return Discount.findOne({ discountCode: discount }).orFail(
      new Error("No docs found!")
    );
  };
...
app.post("/mailchimp", (req, res) => {
  ...
  request(options, function(error, response, body) {
    ...
    if (!response.body.exact_matches.members[0]) {
      ...
      response.body.exact_matches.members[0].interests.d5d2641f68 === true
    ) {
      postMailchimp();
    } else {
      res.json({
        message:
          "This email has already been used. If you are getting this promotion for somebody else please use their email"
      });
    }
  ...
  stripe.customers
    ...
    .then(charge => {
      ...
      ExpressCustomer.estimatedDocumentCount({}, function(err, count) {
        var resultObj = {
          message:
            "Payment has been successful. You will receive an email with your voucher and/or upgrade shortly",
          customerCount: count
        };
        ...
        var options = {
          method: "PUT",
          url:
            "https://us15.api.mailchimp.com/3.0/lists/879953e1ab/members/" +
            subscriber.id,
          headers: {
          ...
          body: {
            ...
            interests: {
              "89e3ef05ba": subscriber.freevoucher,
              "0751ff5d8f": subscriber.upgraded,
              d5d2641f68: false
            }
          },
          json: true
