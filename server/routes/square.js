...
router.get('/get-receipt', function (req, res, next) {
	...
	request('https://squareup.com/receipt/preview/cMXC8356kEGLRZfqgdFdeyMF', (error, response, html) => {
		if (!error && response.statusCode === 200) {
			...
			$('head').empty();
			$('head').append('<meta content="width=device-width, initial-scale=1" name="viewport"><meta content="noindex,nofollow" name="robots"><meta charset="utf-8"><title>Receipt from Ananda Spa</title><style>body {background-color: blue;}</style>')
